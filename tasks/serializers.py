...
# NEW: Import the calendar helper
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def _parse_natural_time(self, raw: str):
        ...
        # [Regex Logic for Dates]
        m = re.search(r"\b(\d{1,2})\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s+(\d{2,4})\b", s_lower)
        if m:
            day = int(m.group(1))
        ...
        time_match = re.search(r"(?:at\s*)?((?:1[0-2]|0?[1-9])(?::[0-5]\d)?\s*(?:am|pm))\b", s_lower, flags=re.I)
        if not time_match:
            time_match = re.search(r"(?:at\s*)?((?:[01]?\d|2[0-3]):[0-5]\d)\b", s_lower)
        ...
        if time_match:
            ...
            if ampm:
                ...
                if ampm.group(1).lower() == "pm" and h != 12: h += 12
                if ampm.group(1).lower() == "am" and h == 12: h = 0
            else:
                h, m = map(int, time_str.split(":"))

            due_dt = datetime(base_date.year, base_date.month, base_date.day, h, m, 0, tzinfo=tz)
    ...
    def create(self, validated_data):
        ...
        # 2. Logic for Priority
        lower_title = raw_title.lower()
        urgent_keywords = ['urgent', 'asap', 'important', 'critical', 'deadline']
        if any(keyword in lower_title for keyword in urgent_keywords):
        ...
        # 5. NEW: Send to Google Calendar (if a date was found)
        if due_dt:
            # We run this in a try-except block so calendar errors don't crash the app
            try:
                add_to_google_calendar(task_instance.title, due_dt)
